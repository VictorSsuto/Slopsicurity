...
# pattern_type: "header", "cookie", "body"
TECH_SIGNATURES = [
    # CMS
...
# Known outdated/risky tech worth flagging
RISKY_TECH = {
    "WordPress": (
...
class TechScanner(BaseScanner):
    ...
    def run(self) -> ScanResult:
        ...
        body = resp.text[:50_000]  # cap to first 50 kB

        detected = set()
        for tech, pattern_type, pattern in TECH_SIGNATURES:
            if tech in detected:
                continue
            target_text = {"header": headers_str, "cookie": cookies_str, "body": body}.get(pattern_type, "")
            if re.search(pattern, target_text, re.IGNORECASE):
                detected.add(tech)

        if detected:
            risky = [t for t in detected if t in RISKY_TECH]
            safe = detected - set(risky)
            detail = f"Detected: {', '.join(sorted(detected))}"

            for tech in sorted(risky):
                result.findings.append(Finding(
                    check_id=f"tech_{tech.lower().replace(' ', '_').replace('/', '_')}",
            ...
            if safe:
                result.findings.append(Finding(
                    check_id="tech_detected",
                    category="Technology Detection",
                    label="Technologies Detected",
                    passed=True,
                    score=0,
                    max_score=0,
                    detail=f"Identified (informational): {', '.join(sorted(safe))}",
                ))
        else:
            result.findings.append(Finding(
